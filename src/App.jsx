...
function App() {
  const [activeTab, setActiveTab] = useState('financial') // Default to financial health as per request
        ...
        return (
          <>
            ...
            <section className="hero-wrapper">
              {/* Base gradient layer */}
              <div className="gradient-layer gradient-base"></div>
              {/* Middle gradient layer */}
              <div className="gradient-layer gradient-middle"></div>
              {/* Bottom gradient layers */}
              <div className="gradient-layer gradient-bottom-1"></div>
              <div className="gradient-layer gradient-bottom-2"></div>
              {/* Water splash image overlay gradient */}
              <div className="gradient-layer gradient-water-overlay"></div>
              {/* Wave detail elements */}
              <div className="wave-detail wave-detail-1"></div>
              <div className="wave-detail wave-detail-2"></div>
              <div className="wave-detail wave-detail-3"></div>
              <div className="wave-detail wave-detail-4"></div>
              <div className="wave-detail wave-detail-5"></div>
              <div className="wave-detail wave-detail-6"></div>
            ...
            <section className="pillars-section">
              ...
              <div className="pillars-grid">
                {/* Pillar 1 */}
                <div className="pillar-card" onClick={() => setActiveTab('financial')}>
                ...
                {/* Pillar 3 */}
                <div className="pillar-card" onClick={() => setActiveTab('billing')}>
                  <div className="pillar-icon-circle">
  ...
  return (
    <div className="app-container">
      ...
      <header className="header">
        <div className="utility-exec-title-header" onClick={() => setActiveTab('overview')} style={{ cursor: 'pointer' }}>
          Utility Executive Control
        </div>
        <div className="nav-line-container">
          <div className="nav-line"></div>
          <div className={`nav-active-bar ${activeTab === 'financial' ? 'shift-financial' : ''}`}></div>
        </div>
        <nav className="nav-links">
          <a href="#" className={`nav-link ${activeTab === 'overview' ? 'active' : ''}`} onClick={() => setActiveTab('overview')}>OVERVIEW</a>
          <a href="#" className={`nav-link ${activeTab === 'financial' ? 'active' : ''}`} onClick={() => setActiveTab('financial')}>FINANCIAL HEALTH <span className="arrow-icon">▲</span></a>
          <a href="#" className={`nav-link ${activeTab === 'operations' ? 'active' : ''}`} onClick={() => setActiveTab('operations')}>OPERATIONS & COMPLIANCE <span className="arrow-icon">▲</span></a>
          <a href="#" className={`nav-link ${activeTab === 'billing' ? 'active' : ''}`} onClick={() => setActiveTab('billing')}>BILLING INSIGHTS <span className="arrow-icon">▲</span></a>
        </nav>
      </header>
